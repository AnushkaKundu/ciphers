...
vector<string> matrix(6, string(6, '*'));
...
string findEncryptDigram(vector<string> matrix, string s, map<char, pair<int, int>> &keyVal)
{
    ...
    {
        ret[0] = matrix[c1.first][(c1.second + 1) % 6];
        ret[1] = matrix[c2.first][(c2.second + 1) % 6];
    }
    else if (c1.second == c2.second)
    {
        ret[0] = matrix[(c1.first + 1) % 6][c1.second];
        ret[1] = matrix[(c2.first + 1) % 6][c2.second];
    }
    else
    {
...
string findDecryptDigram(vector<string> matrix, string s, map<char, pair<int, int>> &keyVal)
{
    ...
    if (c1.first == c2.first)
    {
        ret[0] = matrix[c1.first][(c1.second + 5) % 6];
        ret[1] = matrix[c2.first][(c2.second + 5) % 6];
    }
    else if (c1.second == c2.second)
    {
        ret[0] = matrix[(c1.first + 5) % 6][c1.second];
        ret[1] = matrix[(c2.first + 5) % 6][c2.second];
    }
    else
    {
        ret[0] = matrix[c1.first][c2.second];
